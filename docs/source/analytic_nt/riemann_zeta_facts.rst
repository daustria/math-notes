...
.. admonition:: Proposition
	...
	:math:`-\frac{\zeta'}{\zeta}(s)` is analytic on :math:`\sigma > 0` except for a simple pole at :math:`s=1` with residue :math:`1`.

We know :math:`-\frac{\zeta'}{\zeta}(s)` is analytic on this region because :math:`\zeta(s)` is non-zero when :math:`\sigma > 0`. 
By the previous proposition both :math:`\zeta` and :math:`\zeta'` can be taken to be analytic fuctions on :math:`\sigma > 0` 
with the exception of :math:`s=1`. 

We just need to check the pole. Write :math:`\zeta(s)(s-1) = sf(s)` for some function 
:math:`f` analytic on :math:`\sigma > 0`. Differentiating both sides and rearranging yields
